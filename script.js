...
function buildCampusRecommendations(campusKey, text) {
  ...
  const matchedTags = [
    ...new Set(words.map((w) => KEYWORD_TAG_MAP[w]).filter(Boolean)),
  ];

  if (matchedTags.length === 0) return campus.resources.slice(0, 2);

  const hits = campus.resources.filter((r) =>
    (r.tags || []).some((tag) => matchedTags.includes(tag))
  );

  return hits.length ? hits.slice(0, 6) : campus.resources.slice(0, 2);
}
...
function categorize(text) {
  ...
  if (
    hasAny(t, [
      "anxious","anxiety","panic","depressed","depression","sad","hopeless",
      "overwhelmed","stress","stressed","burnout","therapy","counseling",
    ])
  ) categories.push("mental_health");

  if (hasAny(t, ["focus","procrast","motivation","cant start","can't start","stuck","doomscroll"])) {
    categories.push("focus_support");
  }

  if (hasAny(t, ["grade","grades","class","classes","exam","midterm","final","assignment","deadline","homework","study","gpa"])) {
  ...
  const library = {
    ...
    academic_support: {
      ...
      next: [
        ...
        "Ask a professor/TA for the minimum path forward.",
      ],
